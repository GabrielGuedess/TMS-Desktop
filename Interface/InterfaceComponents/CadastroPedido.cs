    ...
    public partial class CadastroPedido : UserControl
    {
        ...
        private void cadastrarPedido_Click(object sender, EventArgs e)
        {
            if (Type.Contains("Cadastrar") && Validation.Validar(contentPedido)){
                ...
                {
                    MessageBox.Show("É necessario cadastrar ao menos uma mercadoria");
                    return;
                }
            ...
            else if (Type.Contains("Update") && Validation.Validar(contentPedido))
            {
                ...
                if(pedido == null)
                {
                    MessageBox.Show("Erro no Updadte");
                    return;
                }
                pedido.ID_pedido = int.Parse(mkPedido.Text);
                if (comboTipoCliente.SelectedIndex == 0)
        ...
        {
            panelMercadoriasAdd.Enabled = true;

            if (Validation.Validar(tableInfoMercadoria))
            {
                Mercadoria mercadoria = new Mercadoria()
                {
                    Descricao = tbDescricao.Text,
                    Valor = tbValor.returnValue(),
                    Volume = tbVolume.returnValue(),
                    Massa = tbPeso.returnValue(),
                    ID_for_cliente = int.Parse(mkPedido.Text)
                };
                comboMercadoriasAdd.Enabled = true;
            }
